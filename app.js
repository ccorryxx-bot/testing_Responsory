...
// ============================================================
// NEW ASYNC showPage (fetch-based)
// ============================================================
async function showPage(nav) {
  ...
  // Hide top area for some pages
  const topArea = document.getElementById('topArea');
  if (nav === 'tasks' || nav === 'agent') topArea.style.display = 'none';
  else topArea.style.display = '';
  ...
  try {
    ...
    if (nav === 'home') {
      if (typeof loadGamesFromDB === 'function') loadGamesFromDB();
    }
    if (nav === 'tasks') {
    ...
    if (nav === 'agent') {
      if (currentUserId && typeof loadDashboardStats === 'function') {
